...
export function EvaluationFlow({ state, dispatch, onComplete }: StepProps) {
  ...
  const handleSaveFlow = () => {
    ...
    dispatch({ type: 'ADD_EVALUATION_FLOW', payload: { id: `flow-${Date.now()}`, name: flowName, steps: finalSteps } });
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="font-headline">Create New Evaluation Flow</CardTitle>
      ...
      <Card>
        <CardHeader><CardTitle className="font-headline">Existing Evaluation Flows</CardTitle></CardHeader>
        <CardContent>
          {state.evaluationFlows.length > 0 ? (
            state.evaluationFlows.map(flow => (
              <div key={flow.id} className="p-4 border rounded-lg mb-4">
                <h3 className="font-bold font-headline mb-2">{flow.name}</h3>
            ...
            <p className="text-center text-muted-foreground">No evaluation flows created yet.</p>
          )}
        </CardContent>
      </Card>
